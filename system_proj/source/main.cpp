...
static const int WND_WIDTH = 1280;
static const int WND_HEIGHT = 800;
...
int main(int, char**)
{
    ...
    ::RegisterClassExW(&wc);
    hwnd = ::CreateWindowW(wc.lpszClassName, L"Dear ImGui DirectX12 Example", WS_POPUP, 100, 100, WND_WIDTH, WND_HEIGHT, nullptr, nullptr, wc.hInstance, nullptr);
    ...
    nestJsProjectCreator = new NestJsProjectCreator("D:\\LOW_CODE", "test", "src");
    ...
    while (done)
    {
        ...
        ImGui::SetNextWindowSize(ImVec2(WND_WIDTH, WND_HEIGHT));

        {
            ...
            ImGui::Text("gamshik count - %f", counter);
...
LRESULT WINAPI WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE:
        if (wParam == MK_LBUTTON) {
            ...
            if (position.x >= 0 &&
                position.x <= WND_WIDTH &&
                position.y >= 0 &&
                position.y <= title_bar_height) {
                SetWindowPos(
                    hwnd,
                    HWND_TOPMOST,
                    rect.left,
